...
const { getLangByExtension, parseContent } = require('../utils/lang-highlight')
...
class File {
  constructor ({ createAt, updateAt, size, path, extension, name }) {
    ...
    this.path = path
    this.name = name
    this.ext = extension.substr(1)
    this.lang = getLangByExtension(this.ext)
    this.fullname = name
    this.isFile = true
    this._page = 0
  ...
  async nextLines () {
    ...
    const text = await readInterval(_begin, _end, this.path)
    if(this.lang) return this.content = parseContent(text)
    this.content = text
  }
